...
The current working slice is the **Python CLI MVP for Linear + Claude Code**. It
polls Linear for active issues, prepares an isolated per-issue workspace, runs a
...
- **Claude CLI** installed and authenticated locally
  ...
  - Authenticate: `claude` (follow the browser login prompt on first run)
- **`gh` CLI** installed and authenticated (required for PR automation)
  - Install: [cli.github.com](https://cli.github.com) or `brew install gh`
  - Authenticate: `gh auth login`
- A repository-owned `WORKFLOW.md`, generated by `symphony init` or written by hand
...
1. **Linear project slug** — the short identifier for your project (e.g. `my-project`).
   ...
   (`linear.app/YOUR-TEAM/project/NAME-SLUG`).
2. **Active / terminal states** — which Linear state names should trigger agent
   work and which mark completion. Defaults match the standard Linear board
   (`Todo`, `In Progress` → `Done`, `Canceled`, `Duplicate`).
   Also prompts for a **workspace root** — the local directory where per-issue
   workspaces are created (defaults to `~/.symphony/workspaces/<project-slug>`).
3. **GitHub org and repo** — the organisation or user name and repository name
   that agents will clone and push PRs to (e.g. `acme-corp` / `my-backend`).
4. **Linear API key** — stored in `~/.config/symphony/credentials.json`.
   Create one at: linear.app/settings/api → Personal API keys.
5. **GitHub token** — a fine-grained personal access token with **Contents** and
...
You can also create or edit `WORKFLOW.md` manually. A minimal Claude Code workflow
looks like this:
...
agent:
  runner: claude_code      # claude_code (default) | codex
...
6. Update the Linear issue state to "In Review".

Use $GITHUB_TOKEN for git authentication and $LINEAR_API_KEY for all Linear API calls.
```

Use workspace hooks when you need each issue workspace to run setup steps:
...
|---|---|---|
| Agent backend | Codex only | Claude Code (default) and Codex now; Gemini, Hermes/Ollama, GPT-Image-1 planned |
| Installation | `mix setup` + CLI | `uv tool` / `pipx` Python CLI now; `.dmg` drag-to-install planned |
| First-run setup | Hand-edit WORKFLOW.md | `symphony init` guided wizard now; desktop GUI planned |
| Linear auth | Personal API key | API key with local credential file now; OAuth 2.0 planned |
...
**Claude Code runner.** The default runner launches Claude Code as a subprocess,
streams rendered issue prompts, handles timeouts, tracks token usage, and cleans
up subprocesses. Agents can push branches, open PRs, and post Linear comments
using injected `LINEAR_API_KEY` and `GITHUB_TOKEN` environment variables.

**Codex app-server runner.** An alternative runner that launches Codex in
...
**Claude Code** (default) — requires the `claude` CLI authenticated locally. Set
...
**Codex** — requires `codex app-server` and uses Codex's JSON-RPC
app-server protocol.
